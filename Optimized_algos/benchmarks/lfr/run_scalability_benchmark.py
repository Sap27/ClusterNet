...
from config import (SCALABILITY_CONFIG, NETWORK_PARAMS, ALGORITHMS,
...
def _classical_row_from_partition_json(
    partition_file: Path,
    G,
    ...
    """Rebuild a classical benchmark row from a saved partition JSON (for --resume)."""
    with open(partition_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    pred_communities = data['communities']
    ...
    return {
        ...
        'success': True,
        'runtime': float('nan'),
        'num_detected': len(pred_communities),
        'num_true': num_true_comms,
        **metrics,
    }


def _run_classical_worker_job(packed):
    """
    Run one (network, algorithm) job in a separate process.

    Must stay at module scope for pickle (Windows spawn). Reloads the graph
    per job so workers do not share large in-memory graphs.
    """
    (
        networks_dir,
        results_dir,
    ...
    return task_index, row


def _filter_networks_by_size_values(
    networks: List[dict],
    only_sizes: Optional[List[float]],
    verbose: bool,
) -> List[dict]:
    """
    Restrict to scalability networks whose metadata ``size`` is in ``only_sizes``.

    Rows without a ``size`` field are kept unchanged (e.g. μ-sweep accuracy runs).
    """
    if not only_sizes:
        return networks
    allowed = frozenset(float(s) for s in only_sizes)
    kept: List[dict] = []
    dropped = 0
    for ni in networks:
        if 'size' not in ni:
            kept.append(ni)
            continue
        if float(ni['size']) in allowed:
            kept.append(ni)
        else:
            dropped += 1
    if verbose and dropped:
        print(
            f"  Size filter {sorted(allowed)}: skipped {dropped} network(s) "
            "that are still in metadata.json but not in this run's size_values."
        )
    return kept
...
def generate_scalability_networks(output_dir: str, config: dict, lfr_dir: str, verbose: bool = True):
    ...
    for n in config['size_values']:
        if verbose:
            print(f"\nGenerating networks for size={n}...")
       
        for realization in range(config['realizations']):
            name = f"n{n:.2f}_r{realization}"
            seed = int(n * 1000) + realization
...
def run_classical_benchmark(
    ...
    resume: bool = False,
    workers: int = 1,
    only_sizes: Optional[List[float]] = None,
):
    """
    ...
    Args:
        ...
        resume: If True, skip (network, algorithm) pairs already recorded in
            classical_accuracy_results.csv with success, or with an existing
            partition JSON under partitions/classical/. Recovered rows from JSON
            get runtime NaN; CSV rows keep stored runtime.
        workers: If > 1, run pending algorithms per network in parallel in this
            many processes (each job reloads the graph from disk). Use 1 for
            sequential execution (default).
        only_sizes: If set, only process networks whose metadata ``size`` is in
            this list (``config['size_values']`` / ``--size-values``). Stops the
            classical pass from walking every size still listed in metadata.json
            after you shrink the benchmark to fewer N.
    """
    networks_path = Path(networks_dir)
    results_path = Path(results_dir)
    ...
    networks_list = _filter_networks_by_size_values(
        metadata['networks'], only_sizes, verbose=verbose
    )
   
    results_csv = results_path / 'classical_accuracy_results.csv'
    existing_df = None
    if resume and results_csv.exists() and results_csv.stat().st_size > 10:
        try:
            existing_df = pd.read_csv(results_csv)
        except (pd.errors.EmptyDataError, pd.errors.ParserError):
            existing_df = None

    def _csv_row_for_pair(net: str, algo: str):
        if existing_df is None or existing_df.empty:
            return None
        sub = existing_df[
            (existing_df['network'] == net) & (existing_df['algorithm'] == algo)
        ]
        if sub.empty:
            return None
        row = sub.iloc[0]
        ...
        if pd.isna(sv):
            ...
            ok = sv.strip().lower() in ('true', '1', 'yes')
        else:
            ok = bool(sv) if isinstance(sv, (bool, np.bool_)) else int(sv) == 1
        if not ok:
            return None
        return row.to_dict()
    ...
    for network_info in networks_list:
        ...
        for algo_name in algorithms:
            ...
            csv_row = _csv_row_for_pair(name, algo_name) if resume else None
            if resume and csv_row is not None:
                if verbose:
                    print(f"  {algo_name}... (skipped, from CSV) ✓")
                results_by_task_id[tid] = csv_row
                continue
            if resume and part_file.is_file():
                try:
                    row = _classical_row_from_partition_json(
                        part_file, G, true_communities, network_info,
                        algo_name, size, num_true_comms,
                    )
                    if verbose:
                        ami = row.get('ami', float('nan'))
                        print(f"  {algo_name}... (skipped, partition file) ✓ AMI={ami:.3f}")
                    results_by_task_id[tid] = row
                    continue
                except (OSError, json.JSONDecodeError, KeyError, ValueError) as e:
                    if verbose:
                        print(
                            f"  {algo_name}... stale partition ({e}), re-running...",
                            end=' ',
                            flush=True,
                        )

            pending.append((tid, algo_name))
        ...
        if nw <= 1:
            ...
            pool = min(nw, len(pending))
            if verbose:
                for _, algo_name in pending:
                    print(f"  Running {algo_name}...")
                print(
                    f"  -> process pool: {pool} worker(s) "
                    f"(min of --workers={nw} and {len(pending)} job(s) for this network)"
                )
            packs = [
                (
                    str(networks_path),
            ...
            with ProcessPoolExecutor(max_workers=pool) as ex:
                futures = [ex.submit(_run_classical_worker_job, p) for p in packs]
                for fut in as_completed(futures):
                    ...
                    if verbose:
                        ...
                        if row.get('success'):
                            ami = row.get('ami', float('nan'))
                            print(f"    ✓ {algo} AMI={ami:.3f}")
                        else:
                            print(f"    ✗ {algo}")

    all_results = [results_by_task_id[i] for i in range(next_task_id)]
    df = pd.DataFrame(all_results)
    df.to_csv(results_path / 'classical_accuracy_results.csv', index=False)
   
    # Summary (scalability uses 'size'; accuracy benchmark uses 'mu')
    ok = df[df['success'] == True]
    if 'mu' in df.columns:
        group_cols = ['mu', 'algorithm']
    elif 'size' in df.columns:
        group_cols = ['size', 'algorithm']
    else:
        group_cols = ['algorithm']
    summary = ok.groupby(group_cols).agg({
        'ami': ['mean', 'std'],
        'nmi': ['mean', 'std'],
        'runtime': ['mean', 'std'],
        'num_detected': 'mean'
...
def run_gnn_scalability_benchmark(
    networks_dir: str,
    results_dir: str,
    gnn_algorithms: list,
    ...
    """
    Run GNN algorithms on scalability networks (varying N, fixed mu).

    Args:
        networks_dir: Directory containing LFR scalability networks
        results_dir: Directory to save results
        gnn_algorithms: Unsupervised GNN + baseline algorithm names
        supervised_algorithms: Semi-supervised algorithm names
        verbose: Print progress
        ...
        only_sizes: If set, restrict to these network sizes
    ...
    csv_path = results_path / 'gnn_scalability_results.csv'
    ...
    for network_info in networks_list:
        name = network_info['name']
        size = network_info.get('size', network_info.get('N', network_info.get('nodes')))
        realization = network_info['realization']
            ...
            if resume:
                ...
                if part_file.is_file():
                    try:
                        ...
                        all_results.append({
                            'network': name, 'size': size, 'realization': realization,
    ...
    if len(df) > 0 and 'ami' in df.columns:
        ...
        if len(success_df) > 0:
            summary = success_df.groupby(['size', 'algorithm']).agg({
                'ami': ['mean', 'std'],
                'nmi': ['mean', 'std'],
                'runtime': ['mean', 'std'],
                'num_detected': 'mean'
            }).round(4)
            summary.to_csv(results_path / 'gnn_scalability_summary.csv')

    if verbose:
        print(f"\nGNN scalability results saved to: {csv_path}")
...
def main():
    ...
    parser.add_argument('--size-values', type=str, default=None,
                        help='Comma-separated network sizes N for scalability (generation + which '
                             'metadata networks classical uses; e.g. "5000" or "500,1000,5000")')
    ...
    parser.add_argument('--resume', action='store_true',
                        help='Skip jobs already in CSV (success) or with a partition JSON')
    parser.add_argument('--workers', type=int, default=1,
                        help='Classical benchmark: parallel worker processes per network (default 1 = sequential)')
    ...
    #     args.size_values=[500,1000,2500,5000,10000,25000,50000,100000]
    # Setup paths
    script_dir = Path(__file__).parent
    ...
    if args.size_values:
        config['size_values'] = [float(x) for x in args.size_values.split(',')] if isinstance(args.size_values, str) else args.size_values
    ...
    if args.all or args.generate:
        ...
        print("="*60)
        print(f"size values: {config['size_values']}")
    ...
    if args.all or args.run_classical:
        ...
        run_classical_benchmark(
            ...
            only_sizes=config['size_values'],
        )
   
    # Run GNN scalability benchmark
    if args.all or args.run_gnn:
        print("\n" + "="*60)
        print("RUNNING GNN ALGORITHMS SCALABILITY BENCHMARK")
        print("="*60)
        ...
        print(f"Size filter: {config.get('size_values', 'all')}")

        run_gnn_scalability_benchmark(
            ...
            resume=args.resume,
            epochs=args.epochs,
            only_sizes=config.get('size_values'),
